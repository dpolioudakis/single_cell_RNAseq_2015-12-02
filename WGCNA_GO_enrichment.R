...
load("../data/WGCNA_3_Modules_CCnormFPM_SP17.rda")

# metDatDF <- read.csv("../data/mmc2.csv", header = TRUE)
# Each column of modulesColors is modules made with different parameters
modulesColors <- modulesColors[ ,2]

# Directory path for output of gene lists for each module to input into GOElite
outModsLists <- "../analysis/GO_enrichment/CCnormFPM_SP17MMS30DS2MEC01"
dir.create(outModsLists)
dir.create(paste(outModsLists, "/lists", sep = ""))
dir.create(paste(outModsLists, "/background", sep = ""))
outGOgraphs <- "../analysis/graphs/WGCNA_GO_CCnormFPM_SP17MMS30DS2MEC01.pdf"
################################################################################
...
background <- AddEnsembl(background)
...
for (i in 1:(nModules - 1)){
  write.table(data.frame(sourceID = modules[[i]][1], systemCode = "En")
              , file = paste(outModsLists, "/lists/", names(modules)[i],".txt", sep = "")
              , row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
}

write.table(data.frame(sourceID = background[1], systemCode = "En")
            , file = paste(outModsLists, "/background/background.txt", sep = "")
            , row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t"
)

# Function to write out gene lists with hgnc gene symbol for specific modules
# Write_Modules <- function (moduleName) {write.table(modules[moduleName]
#               , file = paste(outModsLists, "/", moduleName,".txt", sep = "")
#               , row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
# }
# Write_Modules("MMmidnightblue")
# Write_Modules("MMblue")
################################################################################
